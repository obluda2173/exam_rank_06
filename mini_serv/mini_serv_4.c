...
void remove_client(int fd) {
    FD_CLR(fd, &allset);
    close(fd);
    free(msgs[fd]); // maybe not really sure
    ...
    while (1) {
        ...
        for (int fd = 0; fd <= maxfd; fd++) {
            ...
            if (fd == sockfd) {
                ...
                int new_conn = accept(sockfd, (struct sockaddr *)&servaddr, &socklen);

                if (new_conn >= 0) {
                    register_client(new_conn);
                    break;
                }
            } else {
                int bytes_recv = recv(fd, readbuf, sizeof(readbuf) - 1, 0);

                if (bytes_recv <= 0) {
                    remove_client(fd);
                    break;
                }
                readbuf[bytes_recv] = '\0';
