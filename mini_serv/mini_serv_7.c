...
int ids[65536];
char* msgs[65536];
fd_set writeset, readset, allset;
char readbuf[1024], writebuf[64];
...
void notify_others(int self, char* msg) {
    for (int fd = 0; fd <= maxfd; fd++) {
        if (FD_ISSET(fd, &writeset) && fd != self) {
    ...
    while (1) {
        ...
        for (int fd = 0; fd <= maxfd; fd++) {
            ...
            if (fd == sockfd) {
                ...
                int newconn = accept(sockfd, (struct sockaddr *)&servaddr, &slen);
                ...
                int bytesrcv = recv(fd, &readbuf, sizeof(readbuf) - 1, 0);
